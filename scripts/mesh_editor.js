            ...
            transform, toolsys, cconst)
{
  ...
  var MeshEditor = exports.MeshEditor = class MeshEditor extends screen.Screen {
    constructor(ctx) {
      ...
      this.selectmode = MeshTypes.VERTEX|MeshTypes.HANDLE;
      this.ctx = ctx;
      this.drawlines = [];
    }
    ...
    draw_mesh(mesh, canvas, g) {
      ...
      let dohandles = cconst.ENABLE_HANDLES;
          ...
          for (let l of list) {
            if (first) {
              ...
              if (dohandles) 
                ...
                g.lineTo(l.next.v[0], l.next.v[1]);
            }
          }
      ...
      for (var v of mesh.handles) {
        ...
        g.beginPath();
        g.fillStyle = getElemColor(v, mesh.handles);
        g.rect(v[0]-w*0.5, v[1]-w*0.5, w, w);
        g.fill();
      }
    }
    ...
    on_mousedown(e) {
      ...
      if (this.selectmode & (MeshTypes.VERTEX|MeshTypes.HANDLE) && mesh.hasHighlight()) {
        if (mesh.verts.highlight)
          do_select(mesh.verts);
        else
          do_select(mesh.handles);
      ...
      switch (e.keyCode) {
        case 70: //fkey
          ...
          if (mesh.verts.selected.length == 2) {
            ...
            window.redraw_all();
          } else if (mesh.verts.selected.length > 2) {
            console.log("Make face!");
            let tool = new mesh_tools.CreateFaceOp();
            _appstate.toolstack.execTool(tool);
